...
FROM budget_table2  WHERE MATCH(Program) AGAINST('$program'IN BOOLEAN MODE) GROUP BY Program ");//selects agency level results matching user input - This is a BOOLEAN search
...
"<h4>There are ".$num_rows." Programs mentioning the term $program in current budget year data </h4>";
...
FROM budget_table2 WHERE MATCH(Program) AGAINST('$program' IN BOOLEAN MODE) GROUP BY PROGRAM ");//This query outputs program level results for the agencies that match the above query. This query tells the user how many programs are administered by the agencies that match their input, the program names and totals for each with all forward years funding.

    $num_rows = mysql_num_rows($results_current);
       ($rows = mysql_num_rows($results_current));
	   echo "<h5>There is a total of ".$num_rows." Programs with ".$program." in their name in the current budget year data.</h5>
</a>";
	   for ($j = 0 ; $j < $rows ; ++$j)
 ...
 echo
   "<table class='results'>
   <tr><td class='left'>Agency</td>
<td><a href='agency_results.php?agency=%22".mysql_result($results_current,$j, 'Agency')."%22'   title='Find all Agency results for ".mysql_result($results_current,$j, 'Agency')." 'target='_blank' '>".mysql_result($results_current,$j, 'Agency')."</a>
</td></tr>
<tr>
<td width='30px'>Program</td>
<td><a href='program_results.php?program=%22".mysql_result($results_current,$j, 'Program')."%22'   title='Find all Scheme results for ".mysql_result($results_current,$j, 'Program')." 'target='_blank' '>".mysql_result($results_current,$j, 'Program')."</a>
</td></tr>  
<TR>
...
//////////////////////////////////////////////////////////////////////////////////////
 ...
 {
  echo
  "<p>Sorry there are no Agency names containing the term ".$program.". 
...
"<h4>There are ".$num_rows." $program Agencies in last budget year data </h4>";
    ($rows = mysql_num_rows($result));
...
<tr>
<td width='30px'>Program</td>
<td><a href='program_results.php?program=%22".mysql_result($result,$j, 'Program')."%22'   title='Find all Scheme results for ".mysql_result($result,$j, 'Program')." 'target='_blank' '>".mysql_result($result,$j, 'Program')."</a>
</td></tr>  
<TR>
<td>Last</td><TD class='money'>$".number_format(mysql_result($result,$j, 'sum(last)')).",000  </TD></tr><tr>
<td>Current</td><TD class='money'>$".number_format(mysql_result($result,$j, 'sum(current)')).",000  </TD></tr><tr>
...
FROM budget_table WHERE MATCH(Program) AGAINST('$program' IN BOOLEAN MODE) GROUP BY PROGRAM order by agency");//This query outputs program level results for the agencies that match the above query. This query tells the user how many programs are administered by the agencies that match their input, the program names and totals for each with all forward years funding.

    $num_rows = mysql_num_rows($results);
       ($rows = mysql_num_rows($results));
	   echo "<h5>There is a total of ".$num_rows." Programs with ".$program." in their name in the last budget year data.</h5>
</a>";
	   for ($j = 0 ; $j < $rows ; ++$j)
 ...
 echo
   "<table class='results'>
   <tr><td class='left'>Agency</td>
<td><a href='agency_results.php?agency=%22".mysql_result($results,$j, 'Agency')."%22'   title='Find all Agency results for ".mysql_result($results,$j, 'Agency')." 'target='_blank' '>".mysql_result($results,$j, 'Agency')."</a>
</td></tr>
<tr>
<td width='30px'>Program</td>
<td><a href='program_results.php?program=%22".mysql_result($results,$j, 'Program')."%22'   title='Find all Scheme results for ".mysql_result($results,$j, 'Program')." 'target='_blank' '>".mysql_result($results,$j, 'Program')."</a>
</td></tr>  
<TR>
