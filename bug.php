...
$program = $_GET['program']; 
  ...
  $result= mysql_query("SELECT id,Portfolio,Program,Agency,Objective,value12_13,sum(value12_13),value13_14,sum(value13_14),value14_15,sum(value14_15),value15_16,sum(value15_16),value16_17,sum(value16_17) FROM budget_table2  WHERE MATCH(Program) AGAINST('".$program."'IN BOOLEAN MODE) group by program ");
  $num_rows = mysql_num_rows($result);
 
  echo

  
  "<div class='button'><a href='http://infoaus.net/budget/2013-14/program_results_doc.php?program=$program' target='_blank'>Word</a></div>
...
<tr>
 <TD>
<a href='http://infoaus.net/budget/2013-14/program_results.php?program=%22".mysql_result($result,$j, 'Program')."%22&submit=Show' target='_blank' title='Get Schemes for this Program in new window'>".mysql_result($result,$j, 'Program')."</a></td></tr>
<tr><td class='money'>
...
<p><b>Program:</b> <a href='http://infoaus.net/budget/2013-14/program_results.php?program=%22".mysql_result($result,$j, 'Program')."%22' target='_blank' title='Get Schemes for this Program in new window'>".mysql_result($result,$j, 'Program')."</a></p>
...
$total_current_calc = mysql_query("SELECT value12_13,sum(value13_14) FROM budget_table2");
$num_rows = mysql_num_rows($total_current_calc);
($rows = mysql_num_rows($total_current_calc));
for ($j = 0 ; $j < $rows ; ++$j)
$total_current = "".mysql_result($total_current_calc,$j, 'SUM(value13_14)')."";
...
$query_total_last = mysql_query("SELECT value12_13,sum(value12_13) from `budget_table2`  WHERE MATCH(Program) AGAINST('".mysql_real_escape_string($program)."'IN BOOLEAN MODE) GROUP BY '$program'    ");
...
$query_total_current = mysql_query("SELECT value13_14,sum(value13_14) from `budget_table2`  WHERE MATCH(Program) AGAINST('".mysql_real_escape_string($program)."'IN BOOLEAN MODE) GROUP BY '$program'  ");
...
///////////////////////////////////////////////////
$billion_ = mysql_query("SELECT value13_14,sum(value13_14) from `budget_table2`  WHERE MATCH(Program) AGAINST('".mysql_real_escape_string($program)."'IN BOOLEAN MODE) GROUP BY '$program'   ");
...
include('scripts/tax_totals.php');
  ...
  $result= mysql_query("SELECT Portfolio,Program,Agency,Objective,url,value12_13,sum(value12_13),value13_14,sum(value13_14),value14_15,sum(value14_15),value15_16,sum(value15_16),value16_17,sum(value16_17)  from `budget_table2`  WHERE MATCH(Program) 
...
<td>Scheme</td>
<td class='search'><a href='scheme_results.php?scheme=%22".mysql_result($result,$j, 'Objective')."%22' target='_blank' title=' Get totals for ".mysql_result($result,$j, 'Objective')." - opens in new window'>".mysql_result($result,$j, 'Objective')."</a></TD>
</TR>
<TR>
...
<td>2016/17</td><TD class='money'>$".number_format(mysql_result($result,$j, 'sum(value16_17)')).",000  </td></tr><tr>
...
<td class='source'><a href=" .mysql_result($result,$j, 'URL').' target="_blank" title="Opens in new window">' .mysql_result($result,$j, 'Source')."</a> </TD>
 ...
 $result_scheme= mysql_query("SELECT Portfolio,Program,Agency,Objective,url,value12_13,value13_14,value14_15,value15_16,value16_17  from `budget_table2`  WHERE MATCH(Program) 
  AGAINST('$program'IN BOOLEAN MODE)  ");
...
$scheme_results =  mysql_query("SELECT Agency,Program,Objective from budget_table2 WHERE objective LIKE('%".$program."%') ");
